...
def pingpong(n):
    ...
    def count(ppv, move, index):
        if index == n:
            return ppv
        elif num_eights(index) > 0 or index%8 == 0:
            return count(ppv-move, -move, index+1)
        else:
            return count(ppv+move, move, index+1)
    return count(1,1,1)
...
def missing_digits(n):
    ...
    def helper(count, n):
        if n//10 == 0:
            ...
            return helper(count-1+(n%10-(n//10)%10), n//10)
        else:
...
def count_coins(change):
    ...
    def co(change, n):
        if  change == 0 :
            ...
            count_now = co(change-n, n) 
            count_next = co(change, descending_coin(n))
            return count_now + count_next
    return co(change,25)
