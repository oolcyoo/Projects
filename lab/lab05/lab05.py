...
def closer_city(lat, lon, city_a, city_b):
    ...
    c1 = sqrt((lat - get_lat(city_a))**2 + (lon - get_lon(city_a))**2)#distance((lat, lon), city_a)
    c2 = sqrt((lat - get_lat(city_b))**2 + (lon - get_lon(city_b))**2)#distance((lat, lon), city_b)
    if c1 < c2:
...
def sprout_leaves(t, leaves):
    ...
    ''' if is_leaf(t):
            for i in leaves:
                return tree(label(t), [tree(i)])
        else:
            for branch in branches(t):
                return tree(label(t), [sprout_leaves(branch,leaves)])'''
