...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    select = [x for x in paragraphs if select(x) == True ]
    
    if k <= len(select) - 1 and k >= 0:
        return select[k]
    else:
        return ''
...
def about(topic):
    ...
    def compare(paragraph):
        paragraph_ls = split(lower(remove_punctuation(paragraph)))
        for x in topic:
            if x in paragraph_ls:
                return True
        return False
    return compare
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == len(reference_words) == 0:
        ...
        for index in range(len(typed_words)):
            if index < len(reference_words) and typed_words[index] == reference_words[index]:
                correct +=1
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for x in valid_words:
            ...
            if diff <= limit:
                candidate.append(x)
                num_mistake.append(diff)
        if len(candidate) == 0:
            return typed_word
        else:
            return candidate[num_mistake.index(min(num_mistake))]
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit == -1:
        ...
        substitude = minimum_mewtations(start[1:], goal[1:], limit -1) + 1
...
def time_per_word(words, times_per_player):
    ...
    for w in times_per_player:
        each_time.append([w[x] - w[x-1] for x in range(1, len(w))])
    return match(words, each_time)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    winner = [[] for _ in player_indices]
    for words in word_indices:
        win_time = float('inf')
        player = 0
        for time_i in player_indices:
            if time(match, time_i, words) < win_time:
                win_time = time(match, time_i, words)
                player = time_i
        winner[player].append(word_at(match, words))
    return winner
    # END PROBLEM 10
