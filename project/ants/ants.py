...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown == 0 and self.place.bees[:]:
            any_bee = random_bee(self.place.bees)
            any_bee.health = 0
            any_bee.reduce_health(0)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        place_tracker = self.place.exit
        while place_tracker != None:
            if place_tracker.ant != None:
                place_tracker.ant.buff() 
            place_tracker = place_tracker.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        redo = self.place.entrance
        if self.blocked():
            self.sting(self.place.ant)
        elif self.health > 0:
            if not self.slowed or gamestate.time%2 == 0:
                if not self.scared and destination is not None:
                    self.move_to(destination)
                elif redo is not None and not redo.is_hive:
                    self.move_to(redo)
                    self.scared -= 1
